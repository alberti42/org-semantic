...
It needs nothing from the server, because the run already answers the request
that started it: that reply /is/ the notification, and
=org-semantic-index-finished-functions= is where it arrives. Which is also its
one limit. A run in another process --- a shell, a cron job, another Emacs ---
belongs to a different server, and this one neither knows nor reports it, so
there is nothing to wait for. Searches during such a run answer as they always
did.
