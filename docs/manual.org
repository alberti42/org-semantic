...
| =status=   | =vault= (optional)                                                                       | the release, and which indexes are built and loaded |
...
Ask before anything is running:
...
or over the pipe once it is, where =vault= is optional precisely because the
first question is which binary is on the other end:

#+begin_src json
--> { "jsonrpc": "2.0", "id": 1, "method": "status", "params": {} }
<-- { "version": "0.1.0", "loaded": 0 }
#+end_src

Name a vault and the answer also says which indexes that vault has, and how many
are resident:

#+begin_src json
<-- { "version": "0.1.0", "loaded": 1, "vault": "/home/you/notes",
