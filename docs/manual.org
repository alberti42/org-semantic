...
**vulpea is the companion worth naming here**, because if you keep notes in one
you have that watcher already. [[https://github.com/d12frosted/vulpea][vulpea]]
maintains its own database of what each note *is* --- its =:ID:=, its title, its
tags, what links to it --- and with ~vulpea-db-autosync-mode~ on it watches the
tree to keep that database current. Those watches see exactly what a save hook
cannot, and vulpea publishes what they found as an extension point, so no advice
is involved:
...
The two indexes then answer different halves of one question over one tree ---
vulpea what a note *is*, org-semantic what it *says* --- and neither knows about
the other's directory: =.vulpea/= and =.org-semantic/= sit side by side in the
vault. Note that vulpea reports *per file*, so its own first sync of a thousand
notes fires this a thousand times; the debounce makes that one run.

Without vulpea, the same thing in eleven lines of Emacs's own
=filenotify=. Watch the vault, touch on anything that looks like a note:
...
of a short example: watching a tree means walking it, adding a watch per
directory, and adding one for each directory that appears later. vulpea does that
work already, which is the argument for letting it.
...
current, which is the one thing org-semantic does not do: with
~vulpea-db-autosync-mode~ on, a single ~add-hook~ makes its watches keep *both*
indexes current, including the changes a save hook cannot see. That recipe is
under [[#searching-from-emacs][Searching from Emacs]].
