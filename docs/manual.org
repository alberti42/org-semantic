...
#+begin_example
  the bge-small-en model is not downloaded yet — indexing this vault will fetch it

    [d] Download it — the model comes down first, so this is minutes
    [s] Search by word — needs no model at all, and builds in seconds
    [q] leave it
...
=d= runs the index, which reports the download as it goes and then runs your
search again by itself. =q= or =C-g= leaves it, and declining costs nothing:
=R= indexes the vault whenever you like, which is the same call the offer would
have made. =g= re-runs the search --- worth doing in its own right, since the
notes may have moved on --- and you are asked afresh if it fails again.
=M-x org-semantic-show-status= says the same thing before you ask: a model it
lists as not cached is one that would refuse.
...
keeps a search you re-run from asking the same question again on every reply.
