...
the batch in flight --- measured at a *median of 3.4 seconds* on a full rebuild,
against 9 ms when nothing is running. It answers instead of blocking for the
whole rebuild, which is the point; it is not fast enough to type into. *Lexical
search is unaffected* --- it uses no model --- and stays at a few milliseconds
throughout, which makes it the better fallback while a rebuild is going.
