...
Every release also carries =org-semantic-<version>-src.tar.gz=, and a packager
should pin that rather than the =Source code (tar.gz)= GitHub attaches by
itself. GitHub generates that one on demand and has changed how it compresses
it before, moving the checksum of archives already published; ours is built
with =git archive= and =gzip -n=, so the same tag is the same bytes for ever,
and its hash is in =SHA256SUMS= with the binaries.
