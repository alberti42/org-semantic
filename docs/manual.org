...
over =serve= for a caller holding a session open. The Emacs package is written
and is what [[#searching-from-emacs][Searching from Emacs]] describes: a results
buffer, reindexing as notes are saved, and a binary it can install for you.

The roadmap is org depth rather than more formats. Priority is parsed onto every
passage and has no predicate yet. Following an =[[id:…]]= link to pull a
neighbouring note's context into a passage is not done. On the Emacs side, a
minibuffer interface that narrows as you type is the piece most obviously
missing, and the results buffer is deliberately read-only for now --- writing a
passage back needs a way to be sure a line number from the index still points
where it did.

Known gaps. The two rankings are not fused into one: a phrase or a boolean means
nothing to an embedding, so a combined list would mix hits that honoured your
query with hits that could not. Language detection is right on prose and guesses
on notes that are almost entirely attachment links or shell snippets --- 0.4% of
chunks on the reference vault, and none once the languages are named in the
policy. And a passage cannot be reached across two vaults in one search: a vault
is one argument, and searching several means searching them one at a time.
