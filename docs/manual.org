...
drives ltex-ls-plus over =lsp-mode= and reads the same marker; its
~lsp-ltex-plus-language~ is where the default =en-US= in the policy comes from.
...
nothing in the log. The order counts as a change too, since the first entry is the
default.
