...
| =bytes=              | a download's size; there is no =total= to climb         |
...
reads as a crash. Show a spinner and the size.
...
None of this makes an index cancellable. The server reads its next message only
after finishing the current one, so a =$/cancelRequest= sent during an index is
not seen until the index is over.
