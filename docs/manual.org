...
echo '{ "languages": ["en-US", "it-IT"] }' > bilingual.json

org-semantic index braindump/roam --lexical --config bilingual.json  # 2863 chunks in 0.5s
...
The list of names is empty in one case, and then no setting can be named: the
policy this vault was last indexed under is no longer on disk to compare
against. See [[#config-is-not-derived][=config.json= --- yours, and only a
copy]]. A settings change you made always arrives with its names.

*A policy that reads the same is never refused, even if the index records a
different one.* That happens when a new version of the tool starts taking a
setting into account that the version which built your index ignored. Your
settings did not move, so nothing is rebuilt: the run notes the new state and
carries on. A release that needs a rebuild says so as an out-of-date index
instead, which is a different error with its own remedy.
