...
Hearing about it depends on which way vulpea parses, and it is worth getting right
because the wrong choice fails *silently* --- an index that quietly stops being
updated looks exactly like one that is. With ~vulpea-db-async-extraction~ on, a
background worker does the parsing and reports each file through a declared
extension point, which needs no advice:

#+begin_src emacs-lisp
(add-hook 'vulpea-db-worker-done-functions
          (lambda (_path status _count)
            ;; `applied' is a note written, `missing' one deleted.  The other
            ;; statuses either mean nothing changed or that a further report
            ;; is coming, so there is nothing to reindex for yet.
            (when (memq status '(applied missing))
              (org-semantic-auto-reindex-touch "/Users/you/notes"))))
#+end_src

That variable is *nil by default*, and the hook is the worker's --- so on a stock
vulpea the snippet above is never called and nothing tells you. If you keep the
default synchronous parsing, the signal is ~vulpea-db-update-file~, which is
public and documented but has no hook of its own, so reaching it means
=advice-add= on another package's function --- your call, and the usual caveat
about advice applies. Whichever you use, vulpea reports *per file*: its own first
sync of a thousand notes fires this a thousand times, and the debounce makes that
one run.
...
|--------------------------------------+----------------+-------------------------------------------------------------|
| ~org-semantic-vault-root~            | =nil=          | which vault your notes are in --- above                     |
...
[[#searching-from-emacs][Searching from Emacs]] for how to hear about them, and
for the wrinkle that which signal to use depends on
