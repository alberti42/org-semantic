...
--- the package and the binary are [[#version][one release]] --- but it is where
...
The Emacs package and the binary are one release, shipped from one repository,
so there is one version between them and nothing to negotiate. A client checks
that the binary is the one its own release expects, and installs the right one
if it is not.
