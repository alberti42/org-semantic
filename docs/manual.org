...
line passes with =--config=, written as a plist instead of a JSON file, so arrays
must be *vectors*:

#+begin_src emacs-lisp
(setq org-semantic-config '(:languages ["en-US" "de-DE"] :fold_diacritics t))
#+end_src
...
It is compared *whole*: a partial policy reads as a change to everything it
leaves out. Leave it =nil= --- the default --- and the index is searched as it
stands, which is what the command line does and is never wrong.

The buffer's ten faces are under ~M-x customize-group RET org-semantic-results~.
