...
*A second =SIGINT= ends the process.* The first is honoured only between units
of work, so a wait with no units --- a model download, mid-flight --- does not
see it. The second does.

On the command line none of this applies: one thing is running, so =SIGINT=
ends the program outright.
