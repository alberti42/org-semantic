...
behind --- which is why a link still shows its brackets rather than only its
description.
...
One case comes out wrong, and it is narrower than "a fragment is not a document"
would suggest. A block's opening line is not part of a chunk's text while its
body is, so a passage beginning inside a long block starts at the first body line
and runs to the =#+end_src=: org sees an end marker with no beginning and
fontifies the code as prose. What is *not* a problem --- though both sound as
though they should be --- is folding: the whole passage is fontified before its
tail is hidden, so a hidden =#+end_src= still counts. Nor is emphasis ever cut,
because a paragraph too long for one passage gives every piece of itself the
whole paragraph's span, so what you see is always a complete paragraph.
