...
searches the vault that note belongs to and shows what came back:
...
lab/vacuum.org  ·  2 passages
  0.780 (+2.4σ)  lab / vacuum.org > Bakeout > Turbo pump > line 142 :lab:vac:
...
A hit's address reads left to right and each part is a link: the *directory*
opens in Dired, the *note* opens at its top, the *section* goes to its heading,
and the *line* goes to the passage. Only the leading passage of a section
carries the address --- the ones after it name their line alone, since the path
is unchanged and only the line has moved.
