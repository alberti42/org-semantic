...
| ~org-semantic-wait-for-index~        | =nil=          | wait for a reindex rather than answer from the old index --- below |
...
*Or wait for it, with =org-semantic-wait-for-index=.* Set it and a search for a
vault this Emacs is indexing is not sent at all. It is held, and goes out when
the run replies, so what you read is never a version behind. The buffer says it
is waiting, and the results appear by themselves. Use it when a stale answer is
worse than a slow one --- and remember that a full rebuild is minutes.

A run this Emacs started needs nothing from the server, because the run already
answers the request that started it: that reply /is/ the notification, and
=org-semantic-index-finished-functions= is where it arrives.
