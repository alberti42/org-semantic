...
*The address of a hit is =path= and =line=, and there is no second way to go.*
...
=file= is the same thing made absolute, for convenience. =line= is a line in the
...
One consequence of =line= naming the section: a section long enough to be
divided yields several passages reporting the same =path= and =line=. They are
distinct hits with distinct =text= at one address, which is the right answer
when the jump is to the section.
...
|------------------------------------------------+----------------+---------------------------------------------------------------------|
| =.org-semantic/semantic/<model>/chunks.json=   | ~2 MB          | per chunk: heading path, line, the lines it came from, =:ID:=, tags |
...
| =.org-semantic/tantivy/=                       | ~2 MB          | the lexical index                                                   |
