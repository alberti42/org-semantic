...
One thing outlasts the search timeout, and gives no sign of it: a semantic search
whose *model is not yet downloaded* fetches it first, which is 465 MB for
=e5-small= and 2.2 GB for the large multilingual ones. The search simply stops
for thirty seconds and then says it timed out. This is why it is worth running
=M-x org-semantic-reindex= before the first search on a new machine --- indexing
has two hours to play with, and says what it is downloading.
