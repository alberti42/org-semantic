...
:PROPERTIES:
:CUSTOM_ID: planned-and-not-planned
:END:
** Planned
:PROPERTIES:
:CUSTOM_ID: planned
:END:
New work goes into reading org better. It does not go into reading other file
formats.

- *Search by priority.* A priority is already read from every heading. There is
  no way to search on it yet.
- *Follow an =[[id:…]]= link*, so that a passage can carry some context from the
  note it points at.
- *A search that narrows as you type*, in the minibuffer, beside the results
  buffer that exists now.
- *Editing a note in the results buffer.* The buffer is read-only today. Before
  it can be written to, there has to be a way to check that a line number from
  the index still points at the same line of the note.

** Not planned
:PROPERTIES:
:CUSTOM_ID: not-planned
:END:
Each item below is a decision, not something forgotten. None of them is closed
for good. If you want one of them, open an issue on the
[[https://github.com/alberti42/org-semantic/issues][issue tracker]]. Say what you
were looking for, and why the answer you got was the wrong one.

- *One list from both rankings.* A search is either by meaning or by word, and
  each gives a list of its own. A word search can use a phrase, or =AND=, =OR=
  and =NOT=. A meaning search can use none of them. One combined list would
  therefore mix hits that obeyed your whole query with hits that could not. Such
  lists can be combined all the same: /reciprocal rank fusion/ merges two lists
  by position instead of by score, and [[#markdown-vdb][markdown-vdb]] works
  that way. It is left out until there is evidence, on a real vault, that one
  combined list is better than choosing the right search.
- *Any file format but org.* Only =.org= files are read. Word and OpenDocument
  files, PDFs, Markdown files and images are ignored, wherever they sit in the
  tree. Reading org well is the point of the tool. A tool that reads every format
  ends up reading only headings and paragraphs, because that is all the formats
  have in common.
- *Finding an image by what it shows.* An attached picture is found today only
  through the words near it. To find it by its content needs an image model such
  as CLIP, and a second set of vectors that cannot share this index. It is
  possible. Nobody is working on it.
- *Built-in converters for other formats.* If other formats are ever handled,
  the plan is a plugin rather than a second parser. You would supply a command
  that turns one document --- an =.odt=, say --- into an org note. Everything
  after that is what the tool already does. None of this is written.
- *One search over two vaults.* A search takes one vault. Two vaults means two
  searches.
