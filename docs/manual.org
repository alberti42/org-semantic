...
A run in another process --- a shell, a cron job, another Emacs --- has no reply
here. The server reports it all the same, by reading the vault's lock file, so
=indexing= means /somebody/ is indexing and not merely /we/ are. The search is
then sent again every couple of seconds until the vault is free. That is the one
place anything polls, and it is the only case where there is nothing to wait on.

*A run that nobody finishes therefore holds the search open.* Kill the buffer,
or search again, to stop waiting.
