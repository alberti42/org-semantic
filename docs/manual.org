...
One case comes out wrong, and it is narrow: a kept block with a *blank line* in
it. A blank line ends a paragraph, so such a block is several of them, and only
the first takes the block's opening line into its span --- a passage made of a
later paragraph therefore shows an =#+end_quote= with nothing that opened it, and
org paints the text as prose. A block without a blank line is whole in every
passage of it.

Three things that are *not* problems, though they sound as though they should be.
Folding: the whole passage is fontified before its tail is hidden, so a hidden
=#+end_src= still counts. Emphasis: a paragraph too long for one passage gives
every piece of itself the whole paragraph's span, so what you see is always a
complete paragraph. And a block's opening line, which is inside the span
deliberately --- it is not in the index, and it is in the span so that reading the
note back shows a block that opened.
...
asks which ranking to use --- the prompt names the index each one reads, since
they are two indexes and not two orderings of one --- and two =C-u= ask about
the length of the list as well:
...
#+begin_example
  the bge-small-en model is not downloaded yet
    ...
    [w] Search by word (lexical) — needs no embedding model
    [q] leave it
...
=w= answers this once by word, and leaves the buffer's own ranking alone; if you
