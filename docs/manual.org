...
[[#cache-directory][What is in =.org-semantic/=]] describes each file in it.
...
| path                             | what it is                                                    |
|----------------------------------+---------------------------------------------------------------|
| =semantic/<model>/chunks.json=   | one entry per passage: where it came from, its tags, its =:ID:= |
| =semantic/<model>/vectors.f32=   | one embedding per passage, in the same order                  |
| =semantic/<model>/manifest.json= | what that model's index has seen, and under which settings     |
| =tantivy/=                       | the word index                                                |
| =lexical.json=                   | what the word index has seen, and under which settings         |
| =index.lock=                     | present only while something is indexing this vault           |

There is one directory per embedding model, each complete in itself, so several
models can be built side by side and any one of them removed on its own. The
word index and the meaning index are independent too: rebuilding the word index
takes about a second whatever state the meaning index is in, where re-embedding a
whole vault takes minutes.
