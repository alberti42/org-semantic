...
one. The first three are what locate a hit --- go to the file, then to the
heading or the line --- and they need no cooperation from the notes. Prefer the
heading over the line if the index may be a little behind the buffer: text moves
down a file, a heading does not. =id= is an extra for the vaults that have them.
