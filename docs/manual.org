...
one. All four are anchors on the same passage, and which to prefer is the
client's call, not this tool's --- so all four are sent and none is privileged:

| anchor    | exact                                    | costs                                    |
|-----------+------------------------------------------+------------------------------------------|
| =line=    | yes, when the index matches the buffer   | drifts if text was inserted above it     |
| =heading= | to the node                              | a search, and repeated headings are ambiguous |
| =id=      | index-independent; org resolves it alone | only as precise as the outline is annotated, and many vaults have none |
| =file=    | the note                                 | --- always needed, never sufficient      |
