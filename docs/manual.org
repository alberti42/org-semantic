...
extends what it does with org. Other file formats are under consideration too.
...
- *Other file formats, through converters* :: Only =.org= files are read today.
  ...
  wherever they sit in the tree. The plan is a set of plugins rather than a
  second parser: each plugin turns one document --- an =.odt=, an =.md=, a
  =.pdf= --- into an org note, and everything after that is what the tool
  already does. A tool that reads every format itself ends up reading only
  headings and paragraphs, because that is all the formats have in common.
...
- *Logical connectors for semantic searches* :: A search by word can use a
  phrase, or the =AND=, =OR= and =NOT= connectors. A search by meaning cannot.
  A query becomes one vector there, and the ranking is by distance from it, so
  there is nothing for a connector to act on. The predicates of
  [[#filters][Filters]] are the exception, and they work on both rankings:
  =tag:=, =dir:=, =todo:= and =lang:= all narrow either one, and each of them
  negates with a leading =-=.
- *One list from both rankings* :: The two searches are never combined. Their
  scores share no scale, but that is not the obstacle --- /reciprocal rank
  fusion/ ranks by position in each list rather than by score, so two such lists
  can be merged. The obstacle is the asymmetry above: one combined list would
  mix hits that obeyed your whole query with hits that could not obey the half
  of it a connector expressed. It is left out until there is evidence, on a real
  vault, that one combined list beats choosing the right search.
- *Editing a note in the results buffer* :: The buffer is read-only today. A
  line number comes from the index and can be older than the note. In org a
  line that has drifted lands on =:END:= or =#+end_src=, and those compare equal
  to the line that was meant --- so writing back needs a way to be sure the
  whole passage is still where the index says, not just its first line.
