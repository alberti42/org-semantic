...
number: a warm search is under ten milliseconds, and the first one against a
vault also loads the model. An index is minutes, and is stopped by asking rather
than by waiting --- =M-x org-semantic-cancel=.
