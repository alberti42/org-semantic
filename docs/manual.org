...
which it will be. Two honest limitations: a passage is a *fragment*, and org
fontifies it as though it were a whole document --- emphasis that opened before
the passage began, a block whose =#+end_src= lies out of view, a heading with no
parent in sight --- so it is occasionally wrong and cannot know it. Set the
option to nil for plain text.
