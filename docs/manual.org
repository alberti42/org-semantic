...
an absolute =file=, the =line=, and the =:ID:= when there is one --- enough to
build a clickable link and jump to the passage. Use both: =id= is the nearest
enclosing node's, so it survives the note being moved or renamed but is only as
precise as the outline is annotated, while =line= always points at the passage
itself. Resolving =id= to a file and then going to =line= is right in either
case. =k= and =perFile= are the two caps described under
...
The =:ID:= part of that depends on your notes: it is the nearest one *there is*.
Give each meeting its own and a hit jumps straight to it; put a single =:ID:= at
the top of the file and every hit in it reports that one. =line= is unaffected
either way, so jump by resolving =id= to the file and then going to =line= ---
correct in both cases, and it costs nothing to prefer.
...
#+begin_src json
{
  ...
  "planning": { "semantic": false, "lexical": true },
...
=planning= is a heading's =DEADLINE:= / =SCHEDULED:= / =CLOSED:= line, and is
