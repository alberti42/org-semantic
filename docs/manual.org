...
- *One list from both rankings* :: The two searches are never combined. Their
  scores share no scale, but that is not the real obstacle. /Reciprocal rank
  fusion/ ranks by position in each list rather than by score, so it merges the
  two without one. The obstacle is the asymmetry above: one list would mix hits
  that obeyed your whole query with hits that could not obey the half of it a
  connector expressed. It is left out until there is evidence, on a real vault,
