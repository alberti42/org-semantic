...
exactly as long as the editor does. [[#cancelling][Stopping a run]] is the one
thing that does not travel over the pipe.

Methods:

| method     | params                                                                                   | returns                                       |
|------------+------------------------------------------------------------------------------------------+-----------------------------------------------|
| =search=   | =vault=, =query=, =k=, =perFile=, =mergeBySection=, =mode= (=semantic= or =lexical=), =model=, =any=, =config= | ={"hits": [...]}= |
...
| =status=   | =vault=                                                                                  | the release, and which indexes this vault has |
...
That check needs no server --- it happens before one is started:
...
=status= carries the same string, for a client that is already talking to a
running server and would rather ask than assume. Its own answer is about the
vault: which semantic indexes exist and whether there is a lexical one, so an
editor can offer the commands that will work and explain the ones that will not
rather than failing when they are used.

#+begin_src json
--> { "jsonrpc": "2.0", "id": 1, "method": "status", "params": { "vault": "/home/you/notes" } }
<-- { "version": "0.1.0", "vault": "/home/you/notes", "loaded": 1,
      "semantic": [ { "name": "e5-small", "dim": 384, "about": "100 languages" } ],
      "lexical": true }
#+end_src
...
that is a server-to-client /request/, and answering it would mean reading a
reply from inside a request already being served, which this server does not do.
You are waiting on the id already --- it is the token.
...
Rates are chosen for the pipe, not for your display. Embedding reports *once per
batch*, about every two seconds on a large vault. Scanning and chunking count
files, which is far too fine a unit to send one by one, so they are held to one
report per 100 ms --- except the =last= of a phase, which always goes, so you
never end up rendering 6,400 of 6,522 for ever.
...
Send the server *=SIGINT=* --- =(interrupt-process proc)= from Emacs. It stops
between units of work and answers the request it was serving with LSP's
=-32800= /RequestCancelled/ and =data.kind= of ="cancelled"=. The session
survives: one request ended, not the server.

Not =$/cancelRequest=, and this is the reason: the server reads its next message
only after answering the current one, so a cancellation sent over the pipe would
arrive after the thing it cancels. A signal does not queue behind the work. It
carries no id, which is unambiguous only because requests are served one at a
time --- it stops whatever is running.
