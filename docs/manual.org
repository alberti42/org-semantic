...
Stopping the /work/ and stopping the /program/ are the same act on a command
line, and Ctrl-C has always done both. Behind an editor they are different acts:
you want this reindex abandoned and the server still there for the next query.
That separation is the whole of what follows.
...
*A second =SIGINT= ends the process*, as Ctrl-C always did. The first is only
honoured between units of work, so a wait with no units --- a model download,
mid-flight --- is deaf to it; the second is not.

On the command line none of this applies, and nothing about it changed: with
only one thing to stop there is nothing to separate, so =SIGINT= kills the
program outright as it always has.
