...
| =status=          | =vault=                                                                                  | what this vault has, for a client deciding what to offer --- the fields are below |
| =doctor=          | =vault=                                                                                  | what this vault has and [[#doctor][what is wrong with it]], as sentences for a person |
| =download=        | =model=                                                                                  | fetch that model's weights, and nothing else  |
...
*=status= is a machine reply, and =doctor= is the one written for a person.*
Every field below is a fact for a client to branch on --- whether to offer a
search, whether to say the list is a version behind, whether a download is
needed first. None of it is phrased to be read aloud, which is why there is no
command that prints it: =M-x org-semantic-doctor= answers the same questions in
sentences, and adds what to do about each one.

| field           | means                                                                        |
|-----------------+------------------------------------------------------------------------------|
| =vault=         | the vault asked about, as the server resolved it                             |
| =notes=         | where its notes are --- the vault itself unless [[#index-elsewhere][=.org-semantic-vault.json=]] says otherwise |
| =semantic=      | one entry per model with an index, each carrying =name=, =dim=, =about= and =cached= |
| =cached=        | whether that model is still downloaded here. An index outlives its model, and a search refuses when it is gone |
| =lexical=       | whether the word index exists                                                |
| =loaded=        | whether this vault is resident, so the next search is warm rather than a model load |
| =indexing=      | whether a run is going, *in any process* --- a shell or another Emacs counts |
| =excludeRules=  | how many rules =.org-semantic-ignore= holds. Absent when that file will not read, and absence is the answer: the count used to read =0=, which is also what a vault excluding nothing says |
| =excludeStale=  | whether any index was built under a different list. Absent for the same reason |

Ask =doctor= instead when a person is going to read it.
...
:CUSTOM_ID: planned
:END:
Reading org notes well is the goal of this tool, so most of the future work
extends coverage of org-mode features. Indexing other file formats is under
consideration too, and would be done through a plugin system of converters.

- *Search by priority* :: org's priority cookie (e.g., =[#A]=) is read off every
  heading and stored on every passage. There is no predicate to filter on it
  yet.
- *A search that narrows as you type* :: so that matches are shown in the
  minibuffer, beside the results buffer that exists today.
- *Semantic searches follow =[[id:…]]= links* :: so that a short passage of the
  linked note is indexed beside the link, and carries some of that note's
  context.
- *Find an image by what it shows* :: An attached picture is found today only
  through the words near it. Finding it by its content needs an image model such
  as CLIP, and a second set of vectors that cannot share this index. It is
  possible, and it is a lot more work.
- *Other file formats, through converters* :: Only =.org= files are read today.
  Word and OpenDocument files, PDFs, Markdown files and images are ignored,
  wherever they sit in the vault. The plan is a set of plugins rather than a
  second parser: each plugin turns one document (e.g., an =.odt=, an =.md=, a
  =.pdf=) into an org note, and everything after that is what the tool already
  does. A tool that reads every format itself has two ends, and both are bad: it
  reads only headings and paragraphs, because that is all the formats have in
  common, or it grows into a second pandoc.

** Not planned
:PROPERTIES:
:CUSTOM_ID: not-planned
:END:
If you want one of these, or something not listed here, open an issue on the
[[https://github.com/alberti42/org-semantic/issues][issue tracker]].

- *Logical connectors for semantic searches* :: A search by word can use a
  phrase, or the =AND=, =OR= and =NOT= connectors. A search by meaning cannot. A
  query becomes one vector there, and the ranking is by distance from it, so
  there is nothing for a logical connector to act on. The predicates of
  [[#filters][Filters]] are the exception, as they work for both searches:
  =tag:=, =dir:=, =todo:= and =lang:= all narrow the selection.
- *One list from both rankings* :: The two searches stay separate ---
  [[#two-rankings-never-merged][Two rankings, never merged]] says why, and shows
  it on one query. Their scores share no scale, but that is not the real
  obstacle: /reciprocal rank fusion/ ranks by position in each list rather than
  by score, so it merges two lists without one. What stands in the way is the
  asymmetry above, and it stays out until there is evidence, on a real vault,
  that one combined list beats choosing the right search.
- *Editing a note in the results buffer* :: The buffer is read-only. =RET= opens
  the note at the line under point and goes there. =f= (/follow/) instead shows
  each passage in its note as point moves across the results, and leaves point
  in the list --- so the note is one =C-x o= away. Editing it in that window
  achieves the goal most of the time.
