...
*A semantic search during a semantic rebuild is slow.* There is one embedding
model and the indexer holds it for one batch at a time, so your query waits for
the batch in flight --- *about two seconds* at the worst, against 9 ms when
nothing is running. Many are answered at once, because scanning and chunking need
no model at all; it is the ones landing mid-embedding that wait. It answers
instead of blocking for the whole rebuild, which is the point; it is not fast
enough to type into. *Lexical search is unaffected* --- it uses no model --- and
stays at a few milliseconds throughout, which makes it the better fallback while
a rebuild is going.
