...
the two indexes and nothing besides. [[#what-it-writes][What it writes]] lists
every file in it. All of it is derived, so deleting the directory leaves the
vault exactly as it was: =index --full= writes it again in one pass.
...
*Three more files are yours, and none of them is in that directory.* They sit
beside it, named so they sort together, and nothing can rebuild any of them:
...
*Track them*, in the same place and for the same reason that git keeps
=.gitignore= beside a work tree rather than inside =.git=.

The first two are looked for beside the index directory first and only then
beside the notes, and the first one found is used whole. That is what lets you
index a shared folder your own way. The two are the same directory unless
=.org-semantic-vault.json= says otherwise, so in the ordinary case there is one
of each.

Add =/.org-semantic/= to the vault's =.gitignore=. Everything in it is derived,
so rebuilding is always available and deleting it costs nothing but the time:
=org-semantic index --full= writes it again in one pass.
[[#what-you-may-delete][What you may delete]] says what each file in there costs.
...
:END:
=.org-semantic/= is a cache. Everything in it can be written again by indexing,
so you may delete any of it, or all of it, and lose nothing but the time it
takes to rebuild. [[#what-it-writes][What it writes]] says what each file /is/;
this says what deleting it /costs/.

| delete                             | and it costs                                                                              |
|------------------------------------+-------------------------------------------------------------------------------------------|
| =semantic/<model>/=                | one =index --model <model>=: every passage embedded again, so minutes                     |
| =semantic/<model>/manifest.json=   | the same. That file is what says an index is there, so removing it removes the index       |
...
** The three files you must not delete
:PROPERTIES:
:CUSTOM_ID: files-that-are-yours
:END:
They are not in that directory, and that is the point: nothing can work any of
them out from your notes.

| file                        | delete it and                                                          |
|-----------------------------+------------------------------------------------------------------------|
| =.org-semantic-config.json= | the vault is indexed under the defaults, and the next run is refused until you rebuild |
| =.org-semantic-ignore=      | nothing is left out, so the notes you had excluded come back            |
| =.org-semantic-vault.json=  | the notes are not found at all, and the vault looks empty               |

Keep them in version control with your notes. The last one matters only if the
notes are not beside the index; write it again and everything is as it was, with
no index lost.

Each of the three used to live somewhere worse. The policy was a flag whose value
was copied into the cache directory; the pointer to the notes was inside it too.
Both were files nothing could rebuild, kept in a directory documented as
disposable. If one of the old copies is still there, the run says so and names
where the file belongs now, rather than reading it or ignoring it.
