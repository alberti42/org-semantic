...
Hearing about what those watches found means reaching into vulpea, and the two
signals worth knowing are ~vulpea-db-update-file~, which vulpea calls for a note
whose content is new to it, and its own handler for a removed file, which is the
only place a *deletion* shows up. Neither has a hook, so both mean =advice-add= on
another package --- your call, and the usual caveat about advice applies. Do not
take the first alone and assume you are covered: a note deleted or renamed away
never reaches it, so the index would go on offering a note that is gone, which is
the silent half of being out of date.

If that is more coupling than you want, the =filenotify= watch above is the
honest alternative --- it sees creation, change, deletion and renaming in one
place, at the cost of walking the tree yourself. Either way vulpea reports and
watches *per file*: a first sync of a thousand notes fires this a thousand times,
and the debounce makes that one run.
